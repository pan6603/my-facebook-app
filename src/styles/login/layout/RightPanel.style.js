import styled from "styled-components";


export const RightPanelDiv = styled.div`
    max-width: 650px;
    width: 100%;
    height: 690px;
`

export const FlexboxCentering = styled.div`
    max-width: 650px;
    width: 100%;
    height: 690px;
    display: flex;
    justify-content: center; 
    align-items: center;
`

export const FaceBookLogin = styled.div`
    max-width: 650px;
    width: 100%;
    height: 448px;
`

export const FaceBookLoginFlexbox = styled.div`
    max-width: 650px;
    width: 100%;
    height: 448px;
    display: flex;
    justify-content: center; 
    align-items: center;
`

export const FormWrapper = styled.div`
    max-width: 546px;
    width: 100%;
    height: 448px;
    display: flex;
    flex-direction: column;
    gap: 8px;
`

export const FormTitle = styled.div`
    max-width: 546px;
    width: 100%;
    height: 28px;
`

export const FormTitleText = styled.div`
    width: max-content;
    height: max-content;
    color: #000;
    padding-top: 12px;
    font-size: 18px;
    font-weight: 500;
`

export const InputGroupArea = styled.div`
    max-width: 546px;
    width: 100%;
    height: 404px;
`

export const LoginForm = styled.form`
    max-width: 546px;
    width: 100%;
    height: 404px;
    display: flex;
    flex-direction: column;
`


export const LoginButtonDiv = styled.div`
    max-width: 546px;
    width: 100%;
    height: 56px;
    margin-top: 8px;
`

export const LoginButton = styled.div`
    max-width: 546px;
    width: 100%;
    height: 44px;
    background-color: #2A63D8;
    margin-top: 8px;
    display: flex;
    justify-content: center; 
    align-items: center;
    border-radius: 24px;
    cursor: pointer;

    &:hover {
        background-color: #1F4EB3; /* 조금 더 진한 파랑 */
    }
`

export const LoginTextFlexBox = styled.div`
    max-width: 512px;
    width: 100%;
    height: 9px;
    display: flex;
    justify-content: center; 
    align-items: center;
`

export const LoginText = styled.div`
    width: max-content;
    height: max-content;
    color: white;
    font-size: 15px;
`


export const ForgotPasswordButton = styled.div`
    max-width: 546px;
    width: 100%;
    height: 44px;
    border-radius: 24px;
    margin-top: 8px;

    &:hover {
        background-color: #E4E6EB;
    }    
`

export const ForgotPasswordLink = styled.a`
    max-width: 546px;
    width: 100%;
    height: 44px;
    display: flex;
    justify-content: center; 
    align-items: center;
    cursor: pointer;
`

export const ForgotPwTextDiv = styled.div`
    max-width: 512px;
    width: 100%;
    height: 9px;
    display: flex;
    justify-content: center; 
    align-items: center;
`

export const ForgotPwText = styled.div`
    width: max-content;
    height: max-content;
    display: flex;
    justify-content: center; 
    align-items: center;
`

export const CreateNewAccountButton = styled.div`
    max-width: 546px;
    width: 100%;
    height: 84px;
    display: flex;
    flex-direction: column;
    justify-content: end;
`

export const CreateAccountButton = styled.div`
    max-width: 546px;
    width: 100%;
    height: 44px;
    background-color: #fff;
    border-radius: 24px;
    border: 1px solid #2A63D8;

    &:hover {
        background-color: #E4E6EB;
    }  
`

export const CreateAccountButtonLink = styled.a`
    max-width: 546px;
    width: 100%;
    height: 44px;
    cursor: pointer;
    display: flex;
    justify-content: center; 
    align-items: center;
`

export const AccoutTextDiv = styled.div`
    max-width: 510px;
    width: 100%;
    height: 9px;
    display: flex;
    justify-content: center; 
    align-items: center;
`

export const AccoutText = styled.div`
    width: max-content;
    height: max-content;
    color: #2A63D8;
`